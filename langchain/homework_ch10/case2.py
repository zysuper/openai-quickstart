...
def create_python_repl_tool(debug: bool = False):
    ...
    @tool
    def python_repl(
        code: Annotated[str, "The python code to execute to do your task."],
    ):
...
def run_multi_agent_test(model_factory, debug: bool = False):
    ...
    report_agent = create_agent(
        report_llm,  # 使用 chart_llm 作为图表生成器智能体的语言模型
        [python_repl_tool],  # 图表生成器智能体使用 Python REPL 工具
        tool_message=(
            "Create a clear and organized markdown document with the following sections:"
            " 1. Create a table of the GDP data in markdown format"
            " 2. Generate a line chart visualization of the GDP data and save it as a PNG image"
            " 3. Analyze the GDP trend and provide a brief forecast of future GDP growth"
            " 4. Save the final report as a markdown file"
            " Make sure the document is well-structured and easy to read."
        ),  # 系统消息，指导智能体如何生成图表并保存为 PNG 图片
    ...
    reprot_node = functools.partial(agent_node, agent=report_agent, name="Report_Generator", debug=debug)
    ...
    workflow.add_node("Report_Generator", reprot_node)
...
def main():
    ...
    run_multi_agent_test(lambda: ChatOpenAI(model="gpt-4o-mini"))
    #print(result)
